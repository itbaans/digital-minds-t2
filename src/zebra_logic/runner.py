"""Model loading + generation, via the repo's interpretability plumbing
(same load path as controllability/runner.py)."""
...
@torch.no_grad()
def generate(model, tok, prompt: str, *, max_new_tokens=1024, sample=False, temperature=0.7) -> str:
    device = next(model.parameters()).device
    ids = _chat_template_ids(tok, [{"role": "user", "content": prompt}]).to(device)
